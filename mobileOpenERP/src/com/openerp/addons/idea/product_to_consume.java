...
public class product_to_consume extends BaseFragment {
				...
				public void run() {
					...
					myHandler.post(new Runnable() {
						...
						public void run() {
							...
							btnevent = new Button[OEHelper.mo_product_from_movestock
									.size()];
										...
										btnevent[position]
												.setOnClickListener(new OnClickListener() {
													...
													public void onClick(
														...
														try {
															p =  (Integer) arg0.getTag();
														} catch (Exception e) {
															// TODO Auto-generated catch block
															e.printStackTrace();
														}
														
														QR_scan_for_insert_product_to_consume t=new QR_scan_for_insert_product_to_consume();
...
//													
	...
	public void call_else_method() {
		...
		progressBar.setVisibility(8);
		mListView.setVisibility(View.VISIBLE);
		btnevent = new Button[OEHelper.mo_product_from_movestock.size()];
		if (OEHelper.mo_product_from_movestock.size() > 0) {
			mListView.setAdapter(new ArrayAdapter<String>(getActivity(),
				...
				public View getView(int position, View convertView,
					...
					// TextView txv = (TextView) mView
					// .findViewById(R.id.ref);
					// if (OEHelper.mo_ref_from_movestock.size() > position) {
					// txv.setText(""
					// + OEHelper.mo_ref_from_movestock
					// .get(position));
					// }
					...
					Typeface font = Typeface.createFromAsset(getActivity().getAssets(), "fonts/Georgia.ttf");
					txv1.setTypeface(font);
					if (OEHelper.mo_product_from_movestock.size() > position) {
						txv1.setText(""
								+ OEHelper.mo_product_from_movestock
										.get(position));
					}
					...
					txv2.setTypeface(font);
					if (OEHelper.mo_qty_from_movestock.size() > position) {
						txv2.setText(""
								+ OEHelper.mo_qty_from_movestock.get(position));
					}
					txv2.setTextAppearance(getActivity(),
							android.R.attr.textAppearance);
