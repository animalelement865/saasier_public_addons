...
class stock_picking_out_label(osv.osv):
    ...
    _columns = {
        ...
        'labels_type' : fields.selection([('Label Template','Label Template'),('UPS WorldShip CSV','UPS WorldShip CSV'),('USPS DataPac CSV','USPS DataPac CSV')],'Labels Type')
    }
        ...
        if email_recipients:
            for partner_id in email_recipients.split(','):
                if partner_id:  # placeholders could generate '', 3, 2 due to some empty field values
                    ...
                    email_item = self.pool.get('res.partner').browse(cr, uid, int(partner_id)).email
                    if email_item:
...
class stock_picking_out(osv.osv):
    ...
    _columns = {
        ...
        'cron_label_message':fields.selection([('label_2_b_created','Label waiting for Scheduler'),
                                               ('label_exception', 'Label has Exception'),
...
class stock_picking(osv.osv):
    ...
    def update_label_status(self, cr, uid, ids, context=None):
        ...
        for picking_out_record in self.browse(cr,1,ids):
            ...
            for move_line in picking_out_record.move_lines:
                ...
                logger.notifyChannel("server", netsvc.LOG_INFO, 'Updating Prodlot_id in move_line %s of delivery_order %s' %  (move_line.id, picking_out_record.id))
                ...
                logger.notifyChannel("server", netsvc.LOG_INFO, 'MOVE ID : %s  |||  Tracker: Carrier %s Tracking Number %s' % (move_line.id,move_line.carrier_tracking_ref, picking_out_record.shipping_carrier))
                try:
    ...
    def cron_create_shipment_simplest(self, cr, uid, ids, context=None):
        ...
        ids = stock_picking_out_object.search(cr, uid, [('cron_label_message','in',('label_exception','label_2_b_created'))])
        ...
        for picking_out_record in self.browse(cr,uid,ids):
            ...
            for move_record in picking_out_record.move_lines:
                ...
                #Check if the label of move (pack) was already created with a previous Scheduler, if yes, jump the iteration,
                if move_record.carrier_tracking_ref:
                    continue
                ...
                if picking_out_record.shipping_carrier in ('UPS','USPS','FEDEX'):
                    ...
                    if picking_out_record.partner_id.country_id.code=="US":
                        if move_record.location_id.partner_id.address_validation:
                            try:
                                ...
                                # raise osv.except_osv(_('Source Address Error !'),  _(e))   
                        ...
                        if picking_out_record.partner_id.address_validation:
                            try:
                                ...
                                # raise osv.except_osv(_('Destination Address Error !'),  _(e))   
                                stock_move_object.write(cr,uid,move_record.id,{'faulty': True, 'error_note': e})
                                stock_picking_out_object.write(cr,uid,picking_out_record.id,{'faulty': True, 'error_note': e})
                                continue_picking_too = True 
                                break
                    ...
                    #Create shipements and check if customs are needed or not.
                    if customs_info:
            ...
            if continue_picking_too:
                ...
                if do_not_create_labels:
                    ...
                    stock_picking_out_object.write(cr,uid,picking_out_record.id,{'state':'done',
                                                                                 'last_status_emailed': 'done',
                                                                                 'list_tracking_numbers': email_sting_tracking_numbers,
                                                                                 'cron_label_message':'label_created'})
                    if email_sting_tracking_numbers!='':
    ...
    def create_shipment_simplest(self, cr, uid, ids, context=None):
        ...
        for picking_out_record in self.browse(cr,uid,ids):
            ...
            if continue_picking_too:
                stock_picking_out_object.write(cr,uid,picking_out_record.id,{'state':'done',
                                                                             'cron_label_message': 'label_exception',
                                                                             })
            else:
                stock_picking_out_object.write(cr,uid,picking_out_record.id,{'state':'done',
                                                                             'last_status_emailed': 'done',
                                                                             'cron_label_message': 'label_2_b_created',
                                                                             })
