...
instance.web.UserMenu =  instance.web.Widget.extend({
    ...
    do_update: function () {
        ...
        var fct = function() {
            ...
            return self.alive(func(self.session.uid, ["name", "company_id","login_date"])).then(function(res) {
                var topbar_name = res.name;
                var time_date = res.login_date;
                var trim_date= time_date.slice(0,16);
                if(instance.session.debug){
                ...
                $('.log_time span').append(trim_date);
               /*self.$el.find('.oe_topbar_name').click(function() {
                	alert(time_date);
                });*/
